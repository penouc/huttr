...
const api: ElectronAPI = {
  ...
  sendMessage: (
    ...
    storedAttachments?: import("../shared/types").StoredAttachment[],
    options?: import("../shared/types").SendMessageOptions,
  ) =>
    ipcRenderer.invoke(
  ...
  cancelProcessing: (sessionId: string, silent?: boolean) =>
    ipcRenderer.invoke(IPC_CHANNELS.CANCEL_PROCESSING, sessionId, silent),
  killShell: (sessionId: string, shellId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.KILL_SHELL, sessionId, shellId),
  getTaskOutput: (taskId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.GET_TASK_OUTPUT, taskId),
  respondToPermission: (
    sessionId: string,
    requestId: string,
    allowed: boolean,
  ...
  // Window management
  getWindowWorkspace: () =>
    ipcRenderer.invoke(IPC_CHANNELS.GET_WINDOW_WORKSPACE),
  getWindowMode: () => ipcRenderer.invoke(IPC_CHANNELS.GET_WINDOW_MODE),
  openWorkspace: (workspaceId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.OPEN_WORKSPACE, workspaceId),
  openSessionInNewWindow: (workspaceId: string, sessionId: string) =>
    ipcRenderer.invoke(
  ...
  switchWorkspace: (workspaceId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.SWITCH_WORKSPACE, workspaceId),
  closeWindow: () => ipcRenderer.invoke(IPC_CHANNELS.CLOSE_WINDOW),
  confirmCloseWindow: () =>
    ipcRenderer.invoke(IPC_CHANNELS.WINDOW_CONFIRM_CLOSE),
  ...
  setTrafficLightsVisible: (visible: boolean) =>
    ipcRenderer.invoke(IPC_CHANNELS.WINDOW_SET_TRAFFIC_LIGHTS, visible),
  ...
  readFileAttachment: (path: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.READ_FILE_ATTACHMENT, path),
  storeAttachment: (sessionId: string, attachment: FileAttachment) =>
    ipcRenderer.invoke(IPC_CHANNELS.STORE_ATTACHMENT, sessionId, attachment),
  generateThumbnail: (base64: string, mimeType: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.GENERATE_THUMBNAIL, base64, mimeType),
  ...
  dismissUpdate: (version: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.UPDATE_DISMISS, version),
  getDismissedUpdateVersion: () =>
    ipcRenderer.invoke(IPC_CHANNELS.UPDATE_GET_DISMISSED),
  onUpdateAvailable: (
    callback: (info: import("../shared/types").UpdateInfo) => void,
  ) => {
    const handler = (
      _event: Electron.IpcRendererEvent,
      info: import("../shared/types").UpdateInfo,
    ) => {
      callback(info);
    };
  ...
  onMenuNewChat: (callback: () => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(IPC_CHANNELS.MENU_NEW_CHAT, handler);
  },
  onMenuOpenSettings: (callback: () => void) => {
    const handler = () => callback();
    ipcRenderer.on(IPC_CHANNELS.MENU_OPEN_SETTINGS, handler);
    return () =>
      ipcRenderer.removeListener(IPC_CHANNELS.MENU_OPEN_SETTINGS, handler);
  },
  onMenuKeyboardShortcuts: (callback: () => void) => {
    const handler = () => callback();
    ipcRenderer.on(IPC_CHANNELS.MENU_KEYBOARD_SHORTCUTS, handler);
    return () =>
      ipcRenderer.removeListener(IPC_CHANNELS.MENU_KEYBOARD_SHORTCUTS, handler);
  },

  // Deep link navigation listener (for external craftagents:// URLs)
  onDeepLinkNavigate: (
    callback: (nav: import("../shared/types").DeepLinkNavigation) => void,
  ) => {
    const handler = (
      _event: Electron.IpcRendererEvent,
      nav: import("../shared/types").DeepLinkNavigation,
    ) => {
      callback(nav);
    };
    ipcRenderer.on(IPC_CHANNELS.DEEP_LINK_NAVIGATE, handler);
    return () =>
      ipcRenderer.removeListener(IPC_CHANNELS.DEEP_LINK_NAVIGATE, handler);
  },

  // Auth
  showLogoutConfirmation: () =>
    ipcRenderer.invoke(IPC_CHANNELS.SHOW_LOGOUT_CONFIRMATION),
  showDeleteSessionConfirmation: (name: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.SHOW_DELETE_SESSION_CONFIRMATION, name),
  logout: () => ipcRenderer.invoke(IPC_CHANNELS.LOGOUT),

  // Onboarding
  getAuthState: () =>
    ipcRenderer
      .invoke(IPC_CHANNELS.ONBOARDING_GET_AUTH_STATE)
      .then((r) => r.authState),
  getSetupNeeds: () =>
    ipcRenderer
      .invoke(IPC_CHANNELS.ONBOARDING_GET_AUTH_STATE)
  ...
  startWorkspaceMcpOAuth: (mcpUrl: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.ONBOARDING_START_MCP_OAUTH, mcpUrl),
  ...
  getExistingClaudeToken: () =>
    ipcRenderer.invoke(IPC_CHANNELS.ONBOARDING_GET_EXISTING_CLAUDE_TOKEN),
  isClaudeCliInstalled: () =>
    ipcRenderer.invoke(IPC_CHANNELS.ONBOARDING_IS_CLAUDE_CLI_INSTALLED),
  runClaudeSetupToken: () =>
    ipcRenderer.invoke(IPC_CHANNELS.ONBOARDING_RUN_CLAUDE_SETUP_TOKEN),
  // Native Claude OAuth (two-step flow)
  startClaudeOAuth: () =>
    ipcRenderer.invoke(IPC_CHANNELS.ONBOARDING_START_CLAUDE_OAUTH),
  exchangeClaudeCode: (code: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.ONBOARDING_EXCHANGE_CLAUDE_CODE, code),
  hasClaudeOAuthState: () =>
    ipcRenderer.invoke(IPC_CHANNELS.ONBOARDING_HAS_CLAUDE_OAUTH_STATE),
  clearClaudeOAuthState: () =>
    ipcRenderer.invoke(IPC_CHANNELS.ONBOARDING_CLEAR_CLAUDE_OAUTH_STATE),

  // Settings - Billing
  getBillingMethod: () =>
    ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_GET_BILLING_METHOD),
  ...
  getModelApiKeys: () =>
    ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_GET_MODEL_API_KEYS),
  setModelApiKey: (modelId: "minimax" | "glm", apiKey: string | null) =>
    ipcRenderer.invoke(
  ...
  setModel: (model: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.SETTINGS_SET_MODEL, model),
  ...
  updateWorkspaceSetting: <K extends string>(
    workspaceId: string,
    key: K,
    value: unknown,
  ) =>
  ...
  writePreferences: (content: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.PREFERENCES_WRITE, content),

  // Session Drafts (persisted input text)
  getDraft: (sessionId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.DRAFTS_GET, sessionId),
  setDraft: (sessionId: string, text: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.DRAFTS_SET, sessionId, text),
  deleteDraft: (sessionId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.DRAFTS_DELETE, sessionId),
  getAllDrafts: () => ipcRenderer.invoke(IPC_CHANNELS.DRAFTS_GET_ALL),

  // Session Info Panel
  getSessionFiles: (sessionId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.GET_SESSION_FILES, sessionId),
  getSessionNotes: (sessionId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.GET_SESSION_NOTES, sessionId),
  setSessionNotes: (sessionId: string, content: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.SET_SESSION_NOTES, sessionId, content),
  watchSessionFiles: (sessionId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.WATCH_SESSION_FILES, sessionId),
  unwatchSessionFiles: () =>
    ipcRenderer.invoke(IPC_CHANNELS.UNWATCH_SESSION_FILES),
  onSessionFilesChanged: (callback: (sessionId: string) => void) => {
    ...
    return () =>
      ipcRenderer.removeListener(IPC_CHANNELS.SESSION_FILES_CHANGED, handler);
  },

  // Sources
  getSources: (workspaceId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.SOURCES_GET, workspaceId),
  createSource: (
    workspaceId: string,
    config: Partial<import("@craft-agent/shared/sources").FolderSourceConfig>,
  ...
  loadPresetTheme: (themeId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.THEME_LOAD_PRESET, themeId),
  getColorTheme: () => ipcRenderer.invoke(IPC_CHANNELS.THEME_GET_COLOR_THEME),
  setColorTheme: (themeId: string) =>
    ipcRenderer.invoke(IPC_CHANNELS.THEME_SET_COLOR_THEME, themeId),
  ...
  onBadgeDraw: (
    ...
    const handler = (
      _event: Electron.IpcRendererEvent,
      data: { count: number; iconDataUrl: string },
    ) => {
      callback(data);
    };
  ...
  onNotificationNavigate: (
    ...
    const handler = (
      _event: Electron.IpcRendererEvent,
      data: { workspaceId: string; sessionId: string },
    ) => {
      callback(data);
    };
