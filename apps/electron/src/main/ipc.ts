...
async function validateFilePath(filePath: string): Promise<string> {
  ...
  const isAllowed = allowedDirs.some(
    (dir) => realPath.startsWith(dir + "/") || realPath === dir,
  );
...
export function registerIpcHandlers(
  ...
  ipcMain.handle(
    ...
    async (_event, slug: string) => {
      const defaultWorkspacesDir = join(
        homedir(),
        ".craft-agent",
        "workspaces",
      );
    ...
    async (event, workspaceId: string) => {
      ...
      if (!updated) {
        ...
        if (win) {
          ...
          windowLog.info(
            `Re-registered window ${event.sender.id} for workspace ${workspaceId}`,
          );
        }
      }
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const callingWorkspaceId = windowManager.getWorkspaceForWindow(
        event.sender.id,
      );
  ...
  ipcMain.handle(
    ...
    async (_event, sessionId: string, silent?: boolean) => {
  ...
  ipcMain.handle(
    IPC_CHANNELS.KILL_SHELL,
    async (_event, sessionId: string, shellId: string) => {
  ...
  // Returns true if the response was delivered, false if agent/session is gone
  ipcMain.handle(
    IPC_CHANNELS.RESPOND_TO_CREDENTIAL,
    async (
      _event,
      sessionId: string,
      requestId: string,
      ...
      switch (command.type) {
        ...
        case "setThinkingLevel":
          ...
          if (!isValidThinkingLevel(command.level)) {
            throw new Error(
              `Invalid thinking level: ${command.level}. Valid values: 'off', 'think', 'max'`,
            );
          }
  ...
  ipcMain.handle(
    IPC_CHANNELS.GET_PENDING_PLAN_EXECUTION,
    async (_event, sessionId: string) => {
      ...
      try {
        ...
        try {
          ...
          if (!thumbnail.isEmpty()) {
            (attachment as { thumbnailBase64?: string }).thumbnailBase64 =
              thumbnail.toPNG().toString("base64");
          }
        } catch (thumbError) {
          // Thumbnail generation failed - this is ok, we'll show an icon fallback
          ipcLog.info(
            "Quick Look thumbnail failed (using fallback):",
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const thumbnail = await nativeImage.createThumbnailFromPath(tempPath, {
          width: 200,
          height: 200,
        });
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const workspaceId = windowManager.getWorkspaceForWindow(
          event.sender.id,
        );
        if (!workspaceId) {
        ...
        const attachmentsDir = getSessionAttachmentsPath(
          workspaceRootPath,
          sessionId,
        );
        ...
        if (attachment.base64) {
          ...
          if (Math.abs(decoded.length - attachment.size) > 100) {
            throw new Error(
              `Attachment corrupted: size mismatch (expected ${attachment.size}, got ${decoded.length})`,
            );
          }
          ...
          if (attachment.type === "image") {
            ...
            const validation = validateImageForClaudeAPI(
              decoded.length,
              imageSize.width,
              imageSize.height,
            );
            ...
            if (validation.needsResize && validation.suggestedSize) {
              ipcLog.info(
                `Resizing image from ${imageSize.width}×${imageSize.height} to ${validation.suggestedSize.width}×${validation.suggestedSize.height}`,
              );
              ...
              if (decoded.length > IMAGE_LIMITS.MAX_SIZE) {
                ...
                if (decoded.length > IMAGE_LIMITS.MAX_SIZE) {
                  throw new Error(
                    `Image still too large after resize (${(decoded.length / 1024 / 1024).toFixed(1)}MB). Please use a smaller image.`,
                  );
                }
              }

              ipcLog.info(
                `Image resized: ${attachment.size} → ${finalSize} bytes (${Math.round((1 - finalSize / attachment.size) * 100)}% reduction)`,
              );
          ...
          throw new Error(
            "Attachment has no content (neither base64 nor text)",
          );
        }
        ...
        try {
          ...
          // Thumbnail generation failed - this is ok, we'll show an icon fallback
          ipcLog.info(
            "Thumbnail generation failed (using fallback):",
            thumbError instanceof Error ? thumbError.message : thumbError,
          );
        }
        ...
        if (attachment.type === "office") {
          ...
          try {
            ...
            throw new Error(
              `Failed to convert "${attachment.name}" to readable format: ${errorMsg}`,
            );
          }
        }
        ...
        if (filesToCleanup.length > 0) {
          ipcLog.info(
            `Cleaning up ${filesToCleanup.length} orphaned file(s) after storage error`,
          );
          await Promise.all(
            filesToCleanup.map((f) => unlink(f).catch(() => {})),
          );
        }
  ...
  ipcMain.handle(
    ...
    async (_event, version: string) => {
      const { setDismissedUpdateVersion } =
  ...
  ipcMain.handle(IPC_CHANNELS.OPEN_URL, async (_event, url: string) => {
    ...
    try {
      ...
      if (
        !["http:", "https:", "mailto:", "craftdocs:"].includes(parsed.protocol)
      ) {
  ...
  ipcMain.handle(IPC_CHANNELS.SHOW_LOGOUT_CONFIRMATION, async () => {
    ...
    const result = await dialog.showMessageBox(window, {
      ...
      detail:
        "All conversations will be deleted. This action cannot be undone.",
  ...
  ipcMain.handle(
    ...
    async (): Promise<BillingMethodInfo> => {
  ...
  ipcMain.handle(
    ...
    async (_event, authType: AuthType, credential?: string) => {
      const manager = getCredentialManager();
      ...
      if (credential) {
        if (authType === "api_key") {
          ...
          const { getExistingClaudeCredentials } =
  ...
  // ============================================================
  // Settings - Model API Keys (Minimax, GLM)
  // ============================================================

  // Get model API keys status (whether keys are configured)
  ipcMain.handle(
    IPC_CHANNELS.SETTINGS_GET_MODEL_API_KEYS,
    async (): Promise<{
      hasMinimaxKey: boolean;
      hasGlmKey: boolean;
      hasZenmuxKey: boolean;
    }> => {
      const manager = getCredentialManager();
      const [minimaxKey, glmKey, zenmuxKey] = await Promise.all([
        manager.getMinimaxApiKey(),
        manager.getGlmApiKey(),
        manager.getZenmuxApiKey(),
      ]);
      return {
        hasMinimaxKey: !!minimaxKey,
        hasGlmKey: !!glmKey,
        hasZenmuxKey: !!zenmuxKey,
      };
    },
  );

  // Set or clear a model API key
  ipcMain.handle(
    IPC_CHANNELS.SETTINGS_SET_MODEL_API_KEY,
    async (_event, modelId: "minimax" | "glm", apiKey: string | null) => {
      const manager = getCredentialManager();

      if (modelId === "minimax") {
        if (apiKey) {
          await manager.setMinimaxApiKey(apiKey);
          ipcLog.info("Minimax API key saved");
        } else {
          await manager.delete({ type: "minimax_api_key" });
          ipcLog.info("Minimax API key cleared");
        }
      } else if (modelId === "glm") {
        if (apiKey) {
          await manager.setGlmApiKey(apiKey);
          ipcLog.info("GLM API key saved");
        } else {
          await manager.delete({ type: "glm_api_key" });
          ipcLog.info("GLM API key cleared");
        }
      } else if (modelId === "zenmux") {
        if (apiKey) {
          await manager.setZenmuxApiKey(apiKey);
          ipcLog.info("Zenmux API key saved");
        } else {
          await manager.delete({ type: "zenmux_api_key" });
          ipcLog.info("Zenmux API key cleared");
        }
      } else {
        throw new Error(`Unknown model ID: ${modelId}`);
      }

      // Reinitialize auth to pick up new API keys
      try {
        await sessionManager.reinitializeAuth();
        ipcLog.info("Reinitialized auth after model API key update");
      } catch (authError) {
        ipcLog.error("Failed to reinitialize auth:", authError);
      }
    },
  ...
  ipcMain.handle(
    ...
    async (_event, workspaceId: string, key: string, value: unknown) => {
      ...
      const { loadWorkspaceConfig, saveWorkspaceConfig } =
        await import("@craft-agent/shared/workspaces");
      const config = loadWorkspaceConfig(workspace.rootPath);
      if (!config) {
      ...
      ipcLog.info(
        `Workspace setting updated: ${key} = ${JSON.stringify(value)}`,
      );
    },
  );
  ...
  ipcMain.handle(
    ...
    async (
      _event,
      workspaceId: string,
      config: Partial<import("@craft-agent/shared/sources").CreateSourceInput>,
    ) => {
      const workspace = getWorkspaceByNameOrId(workspaceId);
  ...
  ipcMain.handle(
    ...
    async (_event, workspaceId: string, sourceSlug: string) => {
      const workspace = getWorkspaceByNameOrId(workspaceId);
      if (!workspace) throw new Error(`Workspace not found: ${workspaceId}`);
  ...
  ipcMain.handle(
    ...
    async (_event, workspaceId: string, sourceSlug: string) => {
      try {
        ...
        const { loadSource, getSourceCredentialManager } =
        ...
        if (
          ...
          return {
            success: false,
            error: "Source not found or not an MCP source",
          };
        }
        ...
        const result = await credManager.authenticate(source, {
          onStatus: (message) =>
            ipcLog.info(`[OAuth] ${source.config.name}: ${message}`),
          onError: (error) =>
            ipcLog.error(`[OAuth] ${source.config.name} error: ${error}`),
        });
  ...
  ipcMain.handle(
    ...
    async (_event, workspaceId: string, sourceSlug: string) => {
      ...
      const { getSourcePermissionsPath } =
  ...
  ipcMain.handle(
    ...
    async (_event, workspaceId: string) => {
      ...
      // Load raw JSON file (not normalized) for UI display
      const { existsSync, readFileSync } = await import("fs");
      const { getWorkspacePermissionsPath } =
        await import("@craft-agent/shared/agent");
    ...
    async (_event, workspaceId: string, sourceSlug: string) => {
      ...
      try {
        ...
        if (source.config.connectionStatus === "failed") {
          return {
            success: false,
            error: source.config.connectionError || "Connection failed",
          };
        }
        ...
        if (source.config.mcp.transport === "stdio") {
          ...
          if (!source.config.mcp.command) {
            return {
              success: false,
              error: 'Stdio MCP source is missing required "command" field',
            };
          }
          ipcLog.info(
            `Fetching MCP tools via stdio: ${source.config.mcp.command}`,
          );
          ...
          if (!source.config.mcp.url) {
            return {
              success: false,
              error: "MCP source URL is required for HTTP/SSE transport",
            };
          }
          ...
          if (
            ...
            source.config.mcp.authType === "bearer"
          ) {
        ...
        const { loadSourcePermissionsConfig, permissionsConfigCache } =
          await import("@craft-agent/shared/agent");
        const permissionsConfig = loadSourcePermissionsConfig(
          workspace.rootPath,
          sourceSlug,
        );
        ...
        const toolsWithPermission = tools.map((tool) => {
          ...
          const allowed = mergedConfig.readOnlyMcpPatterns.some(
            (pattern: RegExp) => pattern.test(tool.name),
          );
          return {
            name: tool.name,
  ...
  ipcMain.handle(
    ...
    async (_event, workspaceId: string, skillSlug: string) => {
      const workspace = getWorkspaceByNameOrId(workspaceId);
      if (!workspace) {
  ...
  ipcMain.handle(
    ...
    async (_event, workspaceId: string, relativePath: string) => {
      const workspace = getWorkspaceByNameOrId(workspaceId);
      if (!workspace) throw new Error("Workspace not found");
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const { writeFileSync, existsSync, unlinkSync, readdirSync } =
        await import("fs");
  ...
  ipcMain.handle(IPC_CHANNELS.THEME_GET_PRESETS, async () => {
    const { loadPresetThemes } =
      await import("@craft-agent/shared/config/storage");
  ...
  ipcMain.handle(
    IPC_CHANNELS.THEME_BROADCAST_PREFERENCES,
    async (
      event,
      ...
      for (const managed of windowManager.getAllWindows()) {
        if (
          ...
          managed.window.webContents.send(
            IPC_CHANNELS.THEME_PREFERENCES_CHANGED,
            preferences,
          );
        }
      }
    ...
    async (_event, enabled: boolean) => {
      ...
      if (enabled) {
        ...
        showNotification(
          ...
          "You will be notified when tasks complete.",
          "",
          "",
        );
      }
